    ...
    case '-':
      if (commenttrigger) {
        ...
        if (result == "SEQUENCE" || result == "SET") {
          string lookforward(readword(definitions_file));
          if (lookforward == "OF")
            result += " OF";
          else
...
void cdrasn1::definitions::preambule(
  ...
  if (workword != "DEFINITIONS") {
    string message("The expected DEFINITIONS statement after the root "
                   "identification (");
    message += root_name + ") wasn't found. Found " + workword + "instead.";
  ...
  if (workword == "IMPLICIT") {
    ...
    string message("The expected type of tags statement after the DEFINITIONS "
  ...
  workword = readword(definitions_file);
  if (workword != "TAGS") {
    string message("The expected TAGS statement after the type wasn't found. "
                   "Found ");
    message += workword + " instead.";
    throw(runtime_error(message));
  }
  workword = readword(definitions_file);
  if (workword != "::=") {
    string message("The expected attribution symbol after the TAGS statement "
                   "wan't found.Found ");
    message += workword + " instead.";
    throw(runtime_error(message));
  }
  workword = readword(definitions_file);
  if (workword != "BEGIN") {
    string message("The expected BEGIN statement after the attribution symbol "
                   "wasn't found. Found ");
    message += workword + " instead.";
    throw(runtime_error(message));
  }
...
cdrasn1::definition_variable cdrasn1::definitions::readvariable(
  ...
  try {
    ...
    string message("Found an error trying to read a new option in the "
                   "definitions file.\n");
    message += err.what();
    throw(runtime_error(message));
  }
  if (definer != "::=") {
    string message("The definer statement wasn't correctly found when reading "
                   "variable: ");
  ...
  if (continuity == "{") {
    ...
    while (!feof(definitions_file) &&
           continuity != "}") { // TODO: Read the options of the custom type
                                              ...
                                              // automático
      {
        ...
        if (result.type == asn1type::integer ||
          ...
          if (nextword.front() == '(') {
            ...
            try {
              value = extract_number_from_brackets(nextword);
            } catch (const runtime_error &err) {
              // TODO: Deal with it
            }
            result.value_options.insert(make_pair(value, workword));
          } else {
            fseek(definitions_file, intern_variable_savepoint, SEEK_SET);
          }
        }
        // TODO: repensar o tipo do resultado para poder
        // referenciar as custom tags.
        else {
